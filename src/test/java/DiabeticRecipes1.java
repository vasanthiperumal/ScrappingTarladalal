

import java.io.IOException;
import java.util.ArrayList;
import java.util.List;

import org.openqa.selenium.By;
import org.openqa.selenium.JavascriptExecutor;
import org.openqa.selenium.WebElement;
import org.testng.annotations.Test;

import com.util.ExcelReader;

public class DiabeticRecipes1 extends BaseTest{


	 
	
	public static ArrayList<String> recipeid = new ArrayList<String>();
	public static ArrayList<String> recipeName = new ArrayList<String>();
	public static ArrayList<String> ingredientList = new ArrayList<String>();
	public static ArrayList<String> prepTime = new ArrayList<String>();
	public static ArrayList<String> cookTime = new ArrayList<String>();
	public static ArrayList<String> prepMethod = new ArrayList<String>();
	public static ArrayList<String> NutrientList = new ArrayList<String>();
	public static ArrayList<String> RecipeUrlList = new ArrayList<String>();
	public static String nutrients ;
	public static String ct;
	public static String s ;
	public static String r; 
	public static String Ingredients ;

	@Test
	public static void recipedetails() throws InterruptedException, IOException 
	{
		driver.findElement(By.name("ctl00$txtsearch")).sendKeys("diabetic");
        driver.findElement(By.id("ctl00_imgsearch")).click();
        driver.findElement(By.xpath("//a[@href='recipes-for-indian-diabetic-recipes-370']")).click();
      
		
		for (int p = 1; p <= 7; p++) 
		{
			System.out.println("onpage" + p);
			String url = "https://tarladalal.com/recipes-for-indian-diabetic-recipes-370?pageindex=" + p;
			driver.get(url);
			Thread.sleep(6000);
			
			List<WebElement> RecipeList = driver.findElements(By.xpath("//div[@class='recipelist']//article"));
			System.out.println("Recipe list count is: " + RecipeList.size());
			Thread.sleep(6000);
	    
	    for ( int k=1; k<=RecipeList.size(); k++) 
	    {
	    	
	    	System.out.println("k loop");
	    	JavascriptExecutor js= (JavascriptExecutor)driver;
			js.executeScript("window.scrollBy(0,5000)","");

			Thread.sleep(6000);
			// recipe id
			WebElement r_id = driver.findElement(By.xpath("(//article[@class='rcc_recipecard'])["+k+"]//span"));
			 s = r_id.getText();
			String formattedrecipeid = s.substring(8, s.length() - 9);
			System.out.println(formattedrecipeid);
			System.out.println(s);
			
			// Recipe name
	        WebElement recipename=driver.findElement(By.xpath("//article["+k+"]//div[@class='rcc_rcpcore']/span[1]/a"));	
	    	 r = recipename.getText();
	    	System.out.println("Recipe Name:" + r);
	        recipename.click();
	        Thread.sleep(1000);
	        	

			// ingredient
			String Ingrediants = driver.findElement(By.xpath("//div[@id='rcpinglist']")).getText();
			System.out.println("Ingrediants are : " + Ingrediants);
			boolean isIngrediant = checkListDiabetic.checkIngrediant1(Ingrediants);

	        // preparation Time
			WebElement Preptime =driver.findElement(By.xpath("//p//time[1]"));
			String pt = Preptime.getText();
			 
			
			// cook time
				WebElement Cooktime = driver.findElement(By.xpath("//p//time[2]")); 
				ct = Cooktime.getText();
			 
				// preparation method
				
				  WebElement Prepmethod =
				  driver.findElement(By.xpath("//div[@id='recipe_small_steps']")); String pm =
				  Prepmethod.getText();
				  
				// nutrients
					
				  try
				  { 
					   nutrients = driver.findElement(By.xpath("//div[@id='rcpnuts']")).getText();
				  
				  }
				  catch(Exception e) 
				  {
					  System.out.println("not available");
				  }
				 

	        	 	String strUrl = driver.getCurrentUrl();
	        	 	RecipeUrlList.add(strUrl);
	        	 	System.out.println("Recipe URL : "+strUrl);
	        	
	        	if (isIngrediant)
	        	{
	        		
					recipeid.add(formattedrecipeid.trim());
					recipeName.add(r);

					ingredientList.add(Ingredients);
					System.out.println("Ingrediants are : " + Ingrediants);

					
					  prepTime.add(pt); System.out.println("Preperation Time is : " + pt);
					 
					
					  try { cookTime.add(ct); System.out.println("Cooking Time is : " +
					  Cooktime.getText()); } catch (Exception e) { cookTime.add("NA"); }
					 
					Thread.sleep(4000);

					// preparation method
					
					  prepMethod.add(pm); System.out.println("Preperation Method is : " +
					  Prepmethod.getText());
					 
					Thread.sleep(4000);
					
					
					  try
					  {
					  
					  NutrientList.add(nutrients); 
					  System.out.println("Nutrient Values are : " + nutrients); 
					  } 
					  catch(Exception e) 
					  {
						  NutrientList.add("NA"); 
					  }
				
				

				}
	        	
	        	String url1 = "https://tarladalal.com/recipes-for-indian-diabetic-recipes-370?pageindex="+p;//;
	        	driver.get(url1);
	        	Thread.sleep(5000);
	    }
		
		 
	        	
		
	    
	    }
	    System.out.println(recipeName.size());
		
		ExcelReader xlreader = new ExcelReader(".\\datafiles\\SmartScrapers.xlsx");
		xlreader.setcelldata("diabetic", 0, 0, "recipeid");
		xlreader.setcelldata("diabetic", 0, 1, "recipename");

		xlreader.setcelldata("diabetic", 0, 2, "ingrediant");

		xlreader.setcelldata("diabetic", 0, 3, "preparation Time");
		xlreader.setcelldata("diabetic", 0, 4, "cook Time");

		xlreader.setcelldata("diabetic", 0, 5, "preparation method");
		xlreader.setcelldata("diabetic", 0, 6, "Nutrients");
				
				for (int a = 0; a <=recipeName.size(); a++) 
				{
					try
					{
					xlreader.setcelldata("diabetic", a+1, 0, recipeid.get(a));
					xlreader.setcelldata("diabetic", a+1, 1, recipeName.get(a));
					xlreader.setcelldata("diabetic", a+1, 2, ingredientList.get(a));

					xlreader.setcelldata("diabetic", a+1, 3, prepTime.get(a));
					xlreader.setcelldata("diabetic", a+1, 4, cookTime.get(a));
					xlreader.setcelldata("diabetic", a+1, 5, prepMethod.get(a)); //
					xlreader.setcelldata("diabetic", a+1, 6, NutrientList.get(a));
				    xlreader.setcelldata("diabetic", a+1, 7, RecipeUrlList.get(a));
					}
					catch(Exception e)
					{
						System.out.println(e.getMessage());
					}

				}

	       
		 
		}
		
	}

	 
			
	
	
	
	